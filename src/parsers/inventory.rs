...
pub fn try_parse(entry: &LogEntry, timestamp: chrono::DateTime<chrono::Utc>) -> Option<GameEvent> {
    ...
    let parsed: serde_json::Value = match serde_json::from_str(json_str) {
        Ok(v) => v,
        Err(e) => {
            ::log::warn!("StartHook inventory: malformed JSON payload: {e}");
            return None;
        }
    };
...
mod tests {
    ...
    /// Helper: extract the JSON payload from a `GameEvent::Inventory` variant.
    fn inventory_payload(event: &GameEvent) -> &serde_json::Value {
        static EMPTY: std::sync::LazyLock<serde_json::Value> =
            std::sync::LazyLock::new(|| serde_json::json!(null));
        match event {
            GameEvent::Inventory(e) => e.payload(),
            _ => &EMPTY,
        }
    }
