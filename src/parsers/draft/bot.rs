...
fn try_parse_pack_presentation(body: &str) -> Option<serde_json::Value> {
    ...
    let parsed: serde_json::Value = match serde_json::from_str(json_str) {
        Ok(v) => v,
        Err(e) => {
            ::log::warn!("DraftStatus PickNext: malformed JSON payload: {e}");
            return None;
        }
    };
...
fn try_parse_draft_pick(body: &str) -> Option<serde_json::Value> {
    ...
    let parsed: serde_json::Value = match serde_json::from_str(json_str) {
        Ok(v) => v,
        Err(e) => {
            ::log::warn!("BotDraft_DraftPick: malformed JSON payload: {e}");
            return None;
        }
    };
...
mod tests {
    ...
    /// Helper: extract the JSON payload from a `GameEvent::DraftBot` variant.
    ///
    /// Returns a static null value if the variant is not `DraftBot`,
    /// which will cause assertion failures that clearly indicate the wrong
    /// variant was produced.
    fn draft_bot_payload(event: &GameEvent) -> &serde_json::Value {
        static EMPTY: std::sync::LazyLock<serde_json::Value> =
            std::sync::LazyLock::new(|| serde_json::json!(null));
        match event {
            GameEvent::DraftBot(e) => e.payload(),
            _ => &EMPTY,
        }
    }
